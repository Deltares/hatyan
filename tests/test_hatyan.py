...
def test_prediction_hasrequiredargs():
    ...
    assert str(e.value) == "prediction() has prediction_perperiod=False, so 'times' argument should not be None"
    
    with pytest.raises(TypeError) as e:
        _ = hatyan.prediction(comp=comp_all)
    assert str(e.value) == "prediction() has prediction_perperiod=True, so 'timestep_min' argument should not be None"
