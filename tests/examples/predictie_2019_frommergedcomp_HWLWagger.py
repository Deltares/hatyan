...
for current_station in stat_list:
    ...
    fig.savefig('prediction_HWLW_%im_%s_main'%(times_pred.step, current_station))
    ...
    if 0: #current_station=='HOEKVHLD':
        ...
        #ts_ext_vali = hatyan.crop_timeseries(ts_ext_vali, slice(dt.datetime(1990,1,5),dt.datetime(1990,1,10)))
    ...
    hatyan.write_tsdia_HWLW(ts_ext=ts_ext_prediction_main, filename='prediction_HWLW_%im_%s_main.dia'%(times_pred.step, current_station))
    hatyan.write_tsdia_HWLW(ts_ext=ts_ext_prediction_345, filename='prediction_HWLW_%im_%s_agger345.dia'%(times_pred.step, current_station))
