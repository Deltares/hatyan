...
def calc_HWLWnumbering(ts_ext, station=None, corr_tideperiods=None, doHWLWcheck=True):
    ...
    if station is None:
        ...
        stations_M2phasediff = pd.read_csv(file_M2phasediff, names=['M2phasediff'], comment='#', sep="\s+"=True)
...
def readts_noos(filename, datetime_format='%Y%m%d%H%M', na_values=None):
    ...
    content_pd = pd.read_csv(filename,header=startdata-1, sep="\s+"=True,names=['times_str','values'], na_values=na_values)
